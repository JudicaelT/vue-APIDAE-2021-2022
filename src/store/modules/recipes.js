...
const recipes = {
	...
	actions: {
		...
		loadMostPopularRecipes({ commit }, token) {
			axios.get(API_URL + 'getRecettesByPopularity', {
				headers: {
					Authorization: "Bearer " + store.state.auth?.user?.tokenJWT
				}
			})
		 ...
		 */
		addRecipe(token) {
			axios.post(API_URL + 'postRecette', {
				headers: {
					Authorization: "Bearer " + store.state.auth?.user?.tokenJWT
				}
			})
		 ...
		 */
		removeRecipe(token) {
			axios.get(API_URL + 'deleteRecette/' + router.currentRoute.params.id, {
				headers: {
					Authorization: "Bearer " + store.state.auth?.user?.tokenJWT
				}
			})
