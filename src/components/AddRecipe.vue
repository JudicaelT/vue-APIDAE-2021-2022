				...
				<form class="d-flex flex-column w-100" action="https://vuejs.org/" method="post">
					...
					<div class="recipe-lines">
						...
						v-for="ingredient in ingredients"
							...
							<div class="col-md-6">
								<div>
									<label class="form-label mt-3" for="ingredientName"><b-icon-x-circle v-on:click="removeIngredient(ingredient.id)" class="text-danger cursor-pointer"></b-icon-x-circle> Nom :</label>
							...
							<div class="col-md-3">
								<div>
									...
									<input class="form-control rounded py-4 border-0 bg-grey" type="text"
										placeholder="ex : kg" name="quantityType" />
								</div>
							</div>
						</div>
					</div>
					<div class="btn btn-outline-dark rounded py-2 my-4" v-on:click="addIngredient(count)">
...
<script>
	export default {
		...
		methods: {
			...
			/**
			 * Add a section with inputs fields to fill an ingredient.
			 * The section is created with an id so that it can be removed
			 * with 'removeIngredient(id)' method
			 */
